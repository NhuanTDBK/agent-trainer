...
2. **Model Understanding:** The LLM will review the architecture and parameters of the chosen embedding model (e.g., `BAAI/bge-base-en-v1.5`) to understand its capabilities and limitations. Use skill `skills/understand-hf-embedding` to access the model's configuration and analyze key parameters such as hidden size, number of layers, attention heads, query/passage prefixes, and pooling strategy. This understanding will inform the fine-tuning approach and hyperparameter choices.
3. **Methodology Comparison:** The LLM will compare fine-tuning methods suitable for the dataset (e.g., MultipleNegativesRankingLoss vs. MatryoshkaLoss, need for hard negatives, etc.).
4. **Scale-Up Strategy:** The LLM will formulate a plan to run small, fast iterations first (e.g., using a subset of data or fewer epochs) to validate the pipeline before executing full-scale, long-running epochs.
5. **Human Review:** **STOP.** The LLM must present this analysis and strategy to the human for approval before proceeding to setup.
...
4. **Run the Baseline:** Execute the first run using default parameters on a _small subset_ of the data.
...
- Ask: _"Do you approve this change for the next notebook run?"_
...
- _Constraint:_ Ensure the training job will not exceed the 24-hour limit.
...
_Repeat the loop: Analyze -> Propose -> Wait for Approval -> Execute -> Record._
